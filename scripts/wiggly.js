...
function setup() {
  ...
  app.ticker.add(() => {
    ...
    c.size.value = _.clamp(c.size.value, c.size.baseValue - c.size.variation, c.size.baseValue + c.size.variation)
    ...
    const getCircle = () => {
      for (let i = 0; i < c.vertexCount; i++) {
        ...
        let x =
          Math.cos(angleRad) *
          c.size.value *
          map(pn.noise(angleDeg, yOffset, 0), 0, 1, 1 - c.noiseStrength, 1 + c.noiseStrength)
